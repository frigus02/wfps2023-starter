...
export function routeData({ params }: RouteDataArgs) {
  return createServerData$(id => {
    const discussionId = Number(id);
    return Promise.all([getDiscussionDetails(discussionId), getDiscussionComments(discussionId)]).then(data => {
...
export default function Discussions() {
  ...
  const reactions = createMemo(() => discussionAndComments()?.discussion.reactionGroups.filter((group) => group.totalCount > 0));
    ...
    <main>
      ...
      <section>
        ...
        <div class="comments">
          ...
          <ul>
            <For each={comments()}>
              {(comment) => <div innerHTML={comment.bodyHTML}></div>}
            </For>  
		      </ul>
        </div>
      </section>
